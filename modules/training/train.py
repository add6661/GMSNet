        ...
        with tqdm.tqdm(total=self.steps) as pbar:
            for i in range(self.steps):
                ...
                for b in range(len(positives_c)):
                    ...
                    feats_rank = torch.cat([m1, m2], dim=0)
                    feats_rank = F.normalize(feats_rank, p=2, dim=1)
                    N = len(m1)
                    pos_pairs = torch.stack([
                        torch.arange(N, device=self.dev),
                        torch.arange(N, device=self.dev) + N,
                    ], dim=-1)

                    num_neg = min(N * 2, 256)
                    neg_i = torch.randint(0, N, (num_neg,), device=self.dev)
                    neg_j = torch.randint(N, N * 2, (num_neg,), device=self.dev)
                    neg_pairs = torch.stack([neg_i, neg_j], dim=-1)
                    loss_rank = loco_ranking_loss(feats_rank, pos_pairs, neg_pairs, tau=1e-2, delta=7.6e-2)
                    loss_items.append((loss_rank * 0.05).unsqueeze(0))

                    if b == 0:
                        acc_coarse_0 = check_accuracy(m1, m2)

                loss_rank_val = loss_rank.item() if 'loss_rank' in dir() else 0.0
                ...
                pbar.set_description( 'Loss: {:.4f} acc_c0 {:.3f} acc_c1 {:.3f} acc_f: {:.3f} loss_c: {:.3f} loss_f: {:.3f} loss_kp: {:.3f} #matches_c: {:d} loss_kp_pos: {:.3f} acc_kp_pos: {:.3f} loss_rank: {:.4f}'.format(
                                                                        loss.item(), acc_coarse_0, acc_coarse, acc_coords, loss_coarse, loss_coord, loss_l1, nb_coarse, loss_kp_pos, acc_pos, loss_rank_val) )
                pbar.update(1)
                ...
                self.writer.add_scalar('Loss/ranking', loss_rank_val, i)
