...
class GMSNet(nn.Module):
	""" 
		Implements the inference module for GMSNet. 
		It supports inference for both sparse and semi-dense feature extraction & matching.
	"""

	def __init__(self, weights = os.path.abspath(os.path.dirname(__file__)) + '/../weights/outdoor.pth', top_k = 4096, detection_threshold=0.05):
		super().__init__()
		self.dev = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
		self.net = GMSNetModel().to(self.dev).eval()
		self.top_k = top_k
	...
	def match_lighterglue(self, d0, d1, min_conf = 0.1):
		"""
			Match GMSNet sparse features with LightGlue (smaller version) -- currently does NOT support batched inference because of padding, but its possible to implement easily.
			input:
	...
	@torch.inference_mode()
	def match_GMSNet(self, img1, img2, top_k = None, min_cossim = -1):
		"""
	...
	@torch.inference_mode()
	def match_GMSNet_star(self, im_set1, im_set2, top_k = None):
		"""
